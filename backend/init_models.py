...
async def init_models():
    ...
    async with AsyncSessionLocal() as session:
        ...
        # 检查是否已有模型
        result = await session.execute(
            select(Model).where(Model.category_id == category.id)
        )
        existing = result.scalars().all()
        if existing:
            print(f"⚠ 【{category.name}】已有 {len(existing)} 个模型，跳过初始化")
            return
        ...
        for model_data in MATHEMATICAL_PROGRAMMING_MODELS:
            model = Model(
                ...
                theory_content=model_data["theory"],
                case_content=model_data["cases"],
                code_content=model_data["code"]
            )
            session.add(model)
